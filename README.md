...
- QuizStats декодируется вручную, а не через автогенерируемый Decodable. Обычная реализация требует, чтобы в сохранённых данных были все текущие поля, и падает с ошибкой, если добавить новое поле после того, как уже накопилась статистика в старом формате. Ручной init(from:) вместо этого подставляет 0 для отсутствующих полей, поэтому статистика игрока переживает обновления приложения и добавление новых полей. 
...
- **Отказоустойчивость сети.** При ошибке сети `promptOfflineFallback` игроку отображается алерт "переключиться на офлайн/ выйти ?" ; мост между колбэками алерта и `async` сделан через `withCheckedContinuation`.
